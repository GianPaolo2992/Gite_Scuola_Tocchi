...
public class ClasseService {
    ...
    public ClasseDTO deleteClasseById(Integer id) {
        ...
        if (classeOPT.isPresent()) {
            ...
            if (!classe.getListaGite().isEmpty()) {

                for (Gita gitaToDelete : classe.getListaGite()) {
                    gitaToDelete.getListaClassi().remove(classe);
                    gitaRepository.save(gitaToDelete);
                }
                classe.getListaGite().clear();
            }
    ...
    @Transactional
    public ClasseDTO deleteAssocateClasseGita(Integer idClasse, Integer idGita) {

        Optional<Classe> classeOPT = classeRepository.findById(idClasse);
        Optional<Gita> gitaOPT = gitaRepository.findById(idGita);

        if (gitaOPT.isPresent() && classeOPT.isPresent()) {

            ClasseDTO classeDissociated = ClasseConverter.ConvertToDTO(classeOPT.get());
            gitaOPT.get().getListaClassi().remove(classeOPT.get());
            classeOPT.get().getListaGite().remove(gitaOPT.get());


            classeRepository.save(classeOPT.get());
            gitaRepository.save(gitaOPT.get());

            return classeDissociated;
